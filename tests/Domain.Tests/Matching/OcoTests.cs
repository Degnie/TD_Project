...
public class OcoTests
{
    ...
    // Buy Limit: se dispara cuando el precio baja hasta el nivel solicitado; Open $60 esta
    // por debajo de los tres niveles, asi que las tres cruzan por gap (RN-03).
    [Fact]
    public void LaRamaQueEjecutaCancelaAtomicamenteALasHermanas()
    {
        var rama1 = new Order { SecuenciaCausal = 1, Side = Side.Buy, Type = OrderType.Limit, Cantidad = 1m, PrecioLimite = 90m };
        var rama2 = new Order { SecuenciaCausal = 2, Side = Side.Buy, Type = OrderType.Limit, Cantidad = 1m, PrecioLimite = 80m };
        var rama3 = new Order { SecuenciaCausal = 3, Side = Side.Buy, Type = OrderType.Limit, Cantidad = 1m, PrecioLimite = 70m };
